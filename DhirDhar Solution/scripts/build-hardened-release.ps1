...
$isccCandidates = @(
    "C:\Users\MSI\AppData\Local\Programs\Antigravity IDE\resources\app\node_modules\innosetup\bin\ISCC.exe",
    "C:\Program Files (x86)\Inno Setup 6\ISCC.exe",
    "C:\Program Files\Inno Setup 6\ISCC.exe",
    "d:\DhirDhar\tools\InnoSetup6\ISCC.exe"
)
$innoSetupExe = $null
...
if (-not $innoSetupExe) {
    ...
    if ($cmd) { $innoSetupExe = $cmd.Source }
}
if (-not $innoSetupExe) {
...
# Publish Updater to isolated staging dir and copy only updater binaries to releaseDir
$updaterPublishDir = "$solutionRoot\Publish\Updater"
if (Test-Path $updaterPublishDir) { Remove-Item $updaterPublishDir -Recurse -Force -ErrorAction SilentlyContinue }
New-Item -ItemType Directory -Path $updaterPublishDir -Force | Out-Null

& dotnet publish "$solutionRoot\src\DhirDhar.Updater\DhirDhar.Updater.csproj" -c Release -r win-x64 --self-contained true -o "$updaterPublishDir"
if ($LASTEXITCODE -ne 0) {
    Write-Error "Updater publish failed!"
}

$updaterFiles = Get-ChildItem -Path $updaterPublishDir -Filter "DhirDharUpdater*"
foreach ($uf in $updaterFiles) {
    Copy-Item -Path $uf.FullName -Destination "$releaseDir\$($uf.Name)" -Force
}
